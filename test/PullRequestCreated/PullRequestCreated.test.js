...
import { assertCalledWith } from '../support/Assertions.js';
import * as github from '@actions/github';
import { PullRequestCreated } from '../../src/PullRequestCreated.js';
...
class TestPullRequestCreated extends PullRequestCreated {
  async findEmails(login) {
    ...
    switch (login) {
      ...
      case 'test-reviewer':
        return ['reviewer@sonarsource.com'];
      case 'renamed':
        return ['unknown@sonarsource.com', 'user@sonarsource.com'];
      case 'renovate[bot]':
        return [];
      default:
        throw new Error(`Scaffolding did not expect login ${login}`);
    }
  }
...
describe('PullRequestCreated', () => {
  ...
  it('is-eng-xp-squad and jira-project fails', async () => {
    ...
    const setFailedCore = mock.fn();
    action.setFailedCore = setFailedCore;
    await action.run();
    assertCalledWith(
      setFailedCore,
      'Action failed: jira-project input is not supported when is-eng-xp-squad is set.',
    );
  });
  it('is-eng-xp-squad and additional-fields fails', async () => {
    ...
    assertCalledWith(
      setFailedCore,
      'Action failed: additional-fields input is not supported when is-eng-xp-squad is set.',
    );
  });
  it('DO NOT MERGE PR title skips the action', async () => {
    ...
    action.log = mock.fn();
    await action.run();
    assertCalledWith(action.log, 'Done');
    assertCalledWith(action.log, "'DO NOT MERGE' found in the PR title, skipping the action.");
  });
  it('No PR skips the action', async () => {
    ...
    await action.run();
    assertCalledWith(action.log, 'Done');
    assert.strictEqual(action.log.mock.callCount(), 1);
  });
  ...
  it('Standalone PR with description', async () => {
    ...
    await runAction(
      'KEY',
      'Standalone PR',
      'Original description, this ignores USER-1234 tickets as those should not be parents',
    );
    assert.deepStrictEqual(logTester.logsParams, [
      'Loading PR #42',
      'findEmails called for test-user',
