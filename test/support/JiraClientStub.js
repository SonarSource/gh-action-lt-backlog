...
export const jiraClientStub = {
  async loadIssue(issueId) {
    switch (issueId) {
      case 'MMF-1111':
        return {
          key: 'MMF-1111',
          fields: { project: { key: 'MMF' }, issuetype: { name: 'Initiative' } },
        };
      case 'EPIC-111':
        return {
          key: 'EPIC-111',
          fields: { project: { key: 'EPIC' }, issuetype: { name: 'Epic' } },
        };
      case 'KEY-1234':
        return {
          key: 'KEY-1234',
          fields: {
            ...
            creator: { displayName: 'Creator of KEY-1234' },
          },
        };
      case 'KEY-5678':
        return {
          ...
          fields: {
            project: { key: 'KEY' },
            issuetype: { name: 'Maintenance' },
            creator: { displayName: 'Jira Tech User GitHub' },
          },
        };
      case 'KEY-5555':
        return {
          key: 'KEY-5555',
          fields: { project: { key: 'KEY' }, issuetype: { name: 'Sub-task' } },
        };
      case 'SUBMIT-1':
        return {
          key: 'SUBMIT-1',
      ...
      case 'SUBMIT-2':
        return {
          ...
          fields: {
            ...
            assignee: {
              accountId: '4242-4242',
              emailAddress: 'user@sonarsource.com',
              displayName: 'User',
            },
          },
        };
      case 'SUBMIT-3':
        return {
          ...
          fields: {
            ...
            issuetype: { name: 'Maintenance' },
            assignee: {
              accountId: '712020:9c105dc5-0493-4d71-83a6-ed21c4ba03c0',
              emailAddress: 'nigel@sonarsource.com',
              displayName: 'Jira False Positive Bot',
            },
          },
        };
      case 'THEME-42':
        return {
          key: 'THEME-42',
          fields: { project: { key: 'THEME' }, issuetype: { name: 'Theme' } },
        };
      case 'FAKE-1234':
        return null;
      default:
        throw new Error(`Scaffolding did not expect ${issueId}`);
    }
  },
  ...
  async findAccountIdFromEmail(email) {
    switch (email) {
      case 'user@sonarsource.com':
        return '1234-account';
      case 'eng.exp@sonarsource.com':
        return '3333-eng-exp-account';
      case 'team.without.evergreen.epics@sonarsource.com':
        return '4444-no-epics-account';
      case 'teamreview.triager@sonarsource.com':
        return '5000-teamreview-triager-account';
      case 'unknown@sonarsource.com':
        return null;
      case 'renovate@renovate.com':
        return null;
      case 'dependabot@dependabot.com':
        return null;
      default:
  ...
  async findTeamByUser(accountId) {
    switch (accountId) {
      case '1234-account':
        return { name: '.NET Squad', id: 'dot-neeet-team' };
      case '2222-no-team':
        return null;
      case '3333-eng-exp-account':
        return JiraTeams.EngineeringExperience;
      case '4444-no-epics-account':
        return { name: 'No Epics Squad', id: 'no-epics-team' };
      default:
        throw new Error(`Scaffolding did not expect accountId ${accountId}`);
    }
  },
  async findTeamByName(accountId) {
    switch (accountId) {
      case 'fallback-team':
        return { name: 'Analysis Processing Squad', id: 'fallback-team' };
      case 'nonexistent-fallback-team':
        return null;
      default:
        throw new Error(`Scaffolding did not expect team name ${accountId}`);
    }
  },
  async findTransition(issueId, transitionName) {
    switch (transitionName) {
      case 'Merge into master':
        return issueId === 'KEY-1111' ? null : { id: '10000', name: transitionName };
      case 'Merge into branch':
        return issueId === 'KEY-1111' ? null : { id: '10001', name: transitionName };
      case 'Merge':
        return { id: '10002', name: transitionName };
      default:
    ...
    switch (jql) {
      case 'issuetype = Epic AND statusCategory != Done AND (summary ~ "KTLO" OR summary ~ "Evergreen") and "Start date[Date]"<=startOfDay() and duedate>=startOfDay() and "Team[Team]"=dot-neeet-team ORDER BY key':
        return [
          ...
          {
            key: 'NET-0000',
            fields: { summary: 'Duplicate epic from same period that should not be used' },
          },
        ];
      ...
      case 'issuetype = Epic AND statusCategory != Done AND (summary ~ "PREQ") and "Start date[Date]"<=startOfDay() and duedate>=startOfDay() and "Team[Team]"=772ea1dc-3574-42bc-a378-7a898d910ebd ORDER BY key':
        return [
          {
            key: 'SC-1000',
            fields: { summary: 'Current SC Review Epic platform-cloud-eng-squad' },
          },
        ];
      case 'issuetype = Epic AND statusCategory != Done AND (summary ~ "PREQ") and "Start date[Date]"<=startOfDay() and duedate>=startOfDay() and "Team[Team]"=6f2e744b-9f09-4c3a-852e-e2f138d1c14f ORDER BY key':
        return [
          {
            key: 'SC-2222',
            fields: { summary: 'Current SC Review Epic platform-cloud-prod-eng-squad' },
          },
        ];
      default:
  ...
  async createIssue(projectKey, summary, additionalFields) {
    console.log(
      `Invoked jira.createIssue('${projectKey}', '${summary}', ${JSON.stringify(additionalFields)})`,
    );
  ...
  async transitionIssue(issueId, transition, fields = null) {
    console.log(
      `Invoked jira.transitionIssue('${issueId}', ${JSON.stringify(transition)}, ${JSON.stringify(fields)})`,
    );
  },
