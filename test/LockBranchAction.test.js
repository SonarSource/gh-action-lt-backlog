...
import { LockBranchAction } from '../src/helpers/LockBranchAction.js';
          ...
          pullRequests: {
            nodes: [
              { id: 'pr-11', number: 11, autoMergeRequest: null },
              {
                id: 'pr-12',
                number: 12,
                autoMergeRequest: { commitHeadline: 'Auto-merge PR #12' },
              },
            ],
            ...
            nodes: [
              {
                id: 'pr-21',
                number: 21,
                autoMergeRequest: { commitHeadline: 'Auto-merge PR #21' },
              },
...
async function runAction(lockValue) {
...
describe('LockBranchAction', () => {
  ...
  it('Lock with slack and additional-message', async () => {
    process.env['INPUT_BRANCH-PATTERN'] = 'unlocked';
  ...
  it('Already in requested state', async () => {
    ...
    assert.deepStrictEqual(logTester.logsParams, [
      'Invoked sendGraphQL list branch protection rules',
      'The branch `unlocked` is already unlocked.',
      'Done',
    ]);
  });
  it('Unlock with auto-merge cancel', async () => {
