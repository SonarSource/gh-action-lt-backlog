...
export class PullRequestClosed extends PullRequestAction {
  async processJiraIssue(pr, issueId) {
    if (this.isEngXpSquad) {
      // Can't auto-close auto-created issues, the reporter is set to the actual user
      if (pr.isBot()) {
        await this.jira.moveIssue(issueId, 'Resolve issue', {
  ...
  async processMerge(issueId) {
    const transition = await this.jira.findTransition(
      issueId,
      this.isReleaseBranch(this.payload.pull_request?.base.ref)
        ? 'Merge into master'
        : 'Merge into branch',
    );
    if (transition == null) {
  ...
  async processClose(issueId) {
    ...
    if (creator === 'Jira Tech User GitHub') {
      await this.jira.moveIssue(issueId, 'Cancel Issue', {
        resolution: { id: this.resolutionId() },
      });
    } else {
...
runAction(import.meta.url, PullRequestClosed);
