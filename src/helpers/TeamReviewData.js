 ...
 */
import { JiraTeams, GitHubTeamSlugs, RootlyScheduleIds } from './TeamConfiguration.js';
...
export class TeamReviewData {
  ...
  static async create(action, pr, issueId, requested_team) {
    ...
    if (candidate && (await this.senderIsFromOutsideTeam(action, candidate))) {
      const assigneeAccountId = await action.jira.findAccountId(
        await action.findRootlyOnCallEmails(candidate.rootlyScheduleId),
      );
      return new TeamReviewData(
        candidate.createReviewTicket,
        await action.loadSenderAccountId(),
        assigneeAccountId,
        candidate.jiraTeam,
        requested_team,
      );
    ...
    if (requested_team?.slug === GitHubTeamSlugs.PlatformCloudEngineering) {
      return {
        ...
        ignoredGitHubTeamSlugs: [
          GitHubTeamSlugs.PlatformCloudEngineering,
          GitHubTeamSlugs.PlatformCloudProductionEngineering,
        ],
      };
      ...
      return {
        ...
        rootlyScheduleId: issueId.startsWith('PREQ-')
          ? RootlyScheduleIds.PlatformEngXpTriager
