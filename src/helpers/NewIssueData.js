 ...
 */
import { JiraTeams } from './TeamConfiguration.js';
...
export class NewIssueData {
  ...
  static async create(jira, pr, inputJiraProject, inputAdditionalFields, accountId, fallbackTeam) {
    ...
    if (projectKey) {
      ...
      const parameters = this.newIssueParameters(
        projectKey,
        parent,
        additionalFields.issuetype?.name ?? 'Maintenance',
      ); // Transfer issuetype name manually, because parameters should have priority due to Sub-task.
      parameters.description = this.parseDescription(pr.body);
      if (parameters.issuetype.name !== 'Sub-task') {
        // These fields cannot be set on Sub-task. Their values are inherited from the parent issue.
      ...
      return new NewIssueData(projectKey, accountId, accountId, {
        ...additionalFields,
        ...parameters,
      });
    } else {
  ...
  static async createForEngExp(jira, pr, accountId) {
    ...
    if (projectKey === 'BUILD') {
      // PREQ is handled by in-Jira automation, with hardcoded value
      parameters.parent = await this.findEvergreenEpic(jira, JiraTeams.EngineeringExperience);
    }
    return new NewIssueData(
      projectKey,
      accountId,
      projectKey === 'PREQ' ? null : accountId,
      parameters,
    ); // GHA-86 Leave PREQ unassigned
  }
  static async createForTeamReview(jira, teamReview) {
    ...
    return new NewIssueData(
      'PREQ',
      teamReview.senderAccountId,
      teamReview.assigneeAccountId,
      parameters,
    );
  }
  ...
  static findMentionedIssues(pr) {
    ...
    console.log(
      mentionedIssues.length > 0
        ? `Found mentioned issues: ${mentionedIssues}`
  ...
  static async findTeam(jira, userAccountId, projectKey, fallbackTeam) {
    ...
    if (projectKey === 'SC') {
      // GHA-293: Lead of SC is in too many squads => issues lands in random board. Bot PRs should fallback to Cloud Engineering, anything else is configurable via 'fallback-team' parameter.
      return JiraTeams.CloudEngineering;
    }
    const project = await jira.loadProject(projectKey);
    console.log(
      `No team found for current user, using ${projectKey} lead ${project.lead.displayName}`,
    );
  ...
  static async findEvergreenEpic(
    jira,
    team,
    summary = 'summary ~ "KTLO" OR summary ~ "Evergreen"',
  ) {
    if (team) {
      const epics = await jira.findIssues(
        `issuetype = Epic AND statusCategory != Done AND (${summary}) and "Start date[Date]"<=startOfDay() and duedate>=startOfDay() and "Team[Team]"=${team.id} ORDER BY key`,
      );
      if (epics.length === 0) {
        console.log(
          `Could not find Evergreen Epic parent for team ${team.name} with ID ${team.id} and Start/Due date in Jira`,
        );
        return null;
      } else {
        console.log(
          `Found ${epics.length} Evergreen Epic(s), using ${epics[0].key} ${epics[0].fields.summary}`,
        );
  ...
  static parseDescription(body) {
    if (body && !/^Part of\s*<!--.*-->\s*$/s.test(body)) {
      // Don't spam Jira with default PR template
