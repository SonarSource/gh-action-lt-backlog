...
export class OctokitAction extends Action {
  ...
  constructor() {
    ...
    this.jira = new JiraClient(
      ...
      JIRA_SITE_ID,
      JIRA_ORGANIZATION_ID,
      this.inputString('jira-user'),
      this.inputString('jira-token'),
    );
  ...
  async loadPullRequest(pull_number) {
    try {
      ...
      return addPullRequestExtensions(
        (await this.rest.pulls.get(this.addRepo({ pull_number }))).data,
      );
    } catch (error) {
  ...
  async findFixedIssues(pr) {
    const text = pr.isRenovate() // We're storing the ID in a comment as a workaround for https://github.com/renovatebot/renovate/issues/26833
      ? (await this.listComments(pr.number)).findLast(x => x.body?.startsWith(RENOVATE_PREFIX))
          ?.body
      : pr.title;
    return [...new Set(text?.match(JIRA_ISSUE_PATTERN) ?? [])];
  }
  ...
  async listTeamMembers(teamSlug) {
    ...
    if (!members) {
      ...
      members = (
        await this.rest.teams.listMembersInOrg({
          org: this.repo.owner,
          team_slug: teamSlug,
          per_page: 100,
        })
      ).data; // We don't need to paginate, 100 is more than enough (for now)
  ...
  async findEmails(login) {
    ...
    try {
      const {
        user: { organizationVerifiedDomainEmails: emails },
  ...
  async findRootlyOnCallEmails(scheduleId) {
    ...
    const emails =
      response?.included.filter(x => x.type === 'users').map(x => x.attributes.email) || [];
  ...
  async loadSenderAccountId() {
    if (this.senderAccountId === undefined) {
      this.senderAccountId = await this.jira.findAccountId(
        await this.findEmails(this.payload.sender?.login),
      );
    }
