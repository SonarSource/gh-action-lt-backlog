...
export class MarkdownTextParser {
  ...
  readBlock() {
    if (this.nextIndex >= this.text.length) {
      ...
      this.text.startsWith('http://', this.nextIndex) ||
      this.text.startsWith('https://', this.nextIndex)
    ) {
...
class NextIndex {
  ...
  minToken(candidate) {
    return this.token < 0 || (candidate >= 0 && candidate < this.token) ? candidate : this.token;
  }
}
