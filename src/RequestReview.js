...
export class RequestReview extends PullRequestAction {
  async processJiraIssue(pr, issueId) {
    ...
    const teamReview = await TeamReviewData.create(
      this,
      pr,
      issueId,
      this.payload.requested_team ?? null,
    );
    await this.processRequestReview(
      pr,
      issueId,
      component,
      this.payload.requested_reviewer ?? null,
      teamReview,
    );
  }
}

runAction(import.meta.url, RequestReview);
