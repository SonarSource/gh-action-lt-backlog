 ...
 */
import { OctokitAction } from './helpers/OctokitAction.js';
import { RENOVATE_PREFIX } from './helpers/Constants.js';
import { NewIssueData } from './helpers/NewIssueData.js';
import { TeamReviewData } from './helpers/TeamReviewData.js';
...
export class PullRequestCreated extends OctokitAction {
  ...
  async processNewJiraIssue(pr, inputJiraProject, inputAdditionalFields) {
    ...
    const data = this.isEngXpSquad
      ...
      : await NewIssueData.create(
          ...
          accountId,
          this.inputString('fallback-team'),
        );
    if (data) {
  ...
  async processAllReviews(pr, issueId) {
    ...
    if (this.payload.pull_request) {
      ...
      await this.processRequestReview(
        pr,
        issueId,
        component,
        this.payload.pull_request.requested_reviewers[0] || null,
        null,
      );
  ...
  async persistIssueId(pr, issueId) {
    if (pr.isRenovate()) {
      // Renovate overrides the PR title back to the original https://github.com/renovatebot/renovate/issues/26833
...
runAction(import.meta.url, PullRequestCreated);
