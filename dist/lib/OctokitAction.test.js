...
`);
    ...
    it('listTeamMembers caches', async () => {
        expect(await sut.listTeamMembers('platform-cloud-engineering-squad')).toHaveLength(2);
        expect(await sut.listTeamMembers('platform-cloud-engineering-squad')).toHaveLength(2);
        expect(await sut.listTeamMembers('platform-cloud-production-engineering-squad')).toHaveLength(2);
        expect(logTester.logsParams).toStrictEqual([
            "Loading members of platform-cloud-engineering-squad",
            "Invoked rest.teams.listMembersInOrg({\"org\":\"SonarSource\",\"team_slug\":\"platform-cloud-engineering-squad\",\"per_page\":100})",
            "Loading members of platform-cloud-production-engineering-squad",
            "Invoked rest.teams.listMembersInOrg({\"org\":\"SonarSource\",\"team_slug\":\"platform-cloud-production-engineering-squad\",\"per_page\":100})"
        ]);
    });
    ...
    itRunsOnlyInCI('findEmails no results', async () => {
        expect(await sut.findEmails('unknown-login')).toEqual([]);
        expect(logTester.logsParams).toStrictEqual([
    ...
    it('findEmails skip bots', async () => {
        expect(await sut.findEmails('any[bot]')).toEqual([]);
        expect(logTester.logsParams).toStrictEqual([
            "Skipping email search for bot: any[bot]",
        ]);
    });
    ...
    it('loadSlackUserIdsByName maps members by first and last name', async () => {
        sut.sendSlackGet = async () => ({
            members: [
                { id: 'U1', profile: { first_name: 'John', last_name: 'Smith' } },
                { id: 'U2', profile: { first_name: 'Jane', last_name: 'Doe' } },
                { id: 'U3', profile: { first_name: 'NoLast' } }, // Skipped: missing last name
                { id: 'U4', profile: { first_name: 'John', last_name: 'Smith' } }, // Ignored: first match wins
            ],
        });
        const users = await sut.loadSlackUserIdsByName();
        expect([...users.entries()]).toStrictEqual([
            ['john smith', 'U1'],
            ['jane doe', 'U2'],
        ]);
    });
    it('loadSlackUserIdsByName follows pagination', async () => {
        const pages = {
            '': { members: [{ id: 'U1', profile: { first_name: 'John', last_name: 'Smith' } }], response_metadata: { next_cursor: 'next' } },
            'next': { members: [{ id: 'U2', profile: { first_name: 'Jane', last_name: 'Doe' } }], response_metadata: { next_cursor: '' } },
        };
        sut.sendSlackGet = async (_url, params) => pages[params.cursor ?? ''];
        const users = await sut.loadSlackUserIdsByName();
        expect([...users.entries()]).toStrictEqual([
            ['john smith', 'U1'],
            ['jane doe', 'U2'],
    ...
    it('loadSlackUserIdsByName returns an empty map when the request fails', async () => {
        sut.sendSlackGet = async () => null;
        const users = await sut.loadSlackUserIdsByName();
    ...
    it('processRequestReview from platform team', async () => {
        ...
        expect(logTester.logsParams).toStrictEqual([
            ...
            "Invoked rest.issues.createComment({\"owner\":\"SonarSource\",\"repo\":\"test-repo\",\"issue_number\":42,\"body\":\"Team Review Jira issue ID: [PREQ-4242](https://sonarsource.atlassian.net/browse/PREQ-4242) platform-cloud-engineering-squad\\n<!--slug: platform-cloud-engineering-squad -->\"})",
    ...
    it('processRequestReview from eng xp', async () => {
        const pr = await sut.loadPullRequest(42);
        await sut.processRequestReview(pr, 'PREQ-42', 'Some Component', null, TeamReviewDataStub.createEngXp('1234-account', '5000-assignee'));
        expect(logTester.logsParams).toStrictEqual([
            "Loading PR #42",
    ...
    describe('findRootlyOnCallEmails', () => {
        ...
        itRunsOnlyInCI('succeeds', async () => {
            ...
            expect(await sut.findRootlyOnCallEmails('c461f921-6c62-4039-8c5e-59500682ccb0')).toEqual([expect.stringContaining('sonar')]);
            expect(logTester.logsParams).toStrictEqual([
                "Finding Rootly On-Call email for schedule: c461f921-6c62-4039-8c5e-59500682ccb0",
        ...
        it('null schedule id', async () => {
            expect(await sut.findRootlyOnCallEmails(null)).toHaveLength(0);
            expect(logTester.logsParams).toStrictEqual([]);
        });
    });
});
