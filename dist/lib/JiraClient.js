...
class JiraClient {
    ...
    async findSprintId(boardId) {
        ...
        if (values.length === 0) {
            ...
            const sprint = values.sort((a, b) => a.endDate.getTime() - b.endDate.getTime()).pop(); // There should be exactly one. If not, use the one ending later in case previous sprint was not closed yet.
