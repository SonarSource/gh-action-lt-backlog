...
export abstract class LockBranchBase extends OctokitAction {
  protected async setLockBranch(lockBranch: boolean, existingRule?: ProtectionRule): Promise<void> {
    const pattern = this.inputString('branch-pattern');
    let rule = existingRule ?? await this.findRule(pattern);
    if (rule) {
      if (rule.lockBranch && !lockBranch) {  // Was locked => unlocking
  ...
  protected async findRule(pattern: string): Promise<ProtectionRule> {
  ...
  protected async updateRule(id: string, lockBranch: boolean): Promise<ProtectionRule> {
    const {
