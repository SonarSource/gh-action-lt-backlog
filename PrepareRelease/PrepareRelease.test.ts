...
const defaultSlackId = (email: string) => `U-${email.split('@')[0]}`;

async function runAction(issues: any[], currentlyLocked: boolean = false, resolveSlackId: (email: string) => string | null = defaultSlackId): Promise<void> {
  const pattern = process.env['INPUT_BRANCH-PATTERN']!;
  const action = new PrepareRelease() as unknown as LockBranchActionStub & OctokitActionStub & { run(): Promise<void> };
  ...
  action.jira.findBoardIssues = async (boardId: number, jql: string) => {
    console.log(`Invoked findBoardIssues(${boardId}, ${jql})`);
    return issues;
  };
  action.findSlackUserId = async (email: string) => resolveSlackId(email);
